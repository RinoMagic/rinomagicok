...
- **#1 Partite eliminate/escluse ancora selezionabili (G2/G3)**: nuovo helper `scoreandlive.py::_propagate_fixture_exclusion(home, away, matchday, excluded)` (match whitespace/case-insensitive) usato da **entrambi** i pulsanti: `PATCH /sal/calendar/fixture/{id}/exclude` ("Escludi") e `DELETE /sal/calendar/fixture/{id}` ("Elimina", che PRIMA non propagava). Marca `excluded`+`postponed_before` su tutte le snapshot NON liquidate di quella giornata in `sv_matchdays` (Survival) e `sal_matchdays` (ScoreAndLive) → la partita sparisce dalle giocate. Per Tiket inietta un fixture `postponed=True` (quota 1.00 neutra) nelle room aperte della giornata; inoltre `thebesttiket.py::leaderboard` legge le escluse da `sal_calendar` e le tratta come quota 1.00 ("ESCL."). Verificato via curl: exclude su MD3 → propagated sv:1 sal:5, partita rimossa; re-include ripristina. Scope corretto per singola giornata.
- **#2 Cuoricini "vite a inizio giornata" nel riepilogo**: `surviva.py::participant_picks` ora restituisce `lives_at_start` per giornata (= n. giocate persistite per le giornate calcolate = vite a inizio giornata, include bonus; = vite attuali per la giornata corrente). `SurvivaPicksModal.js` mostra i cuoricini rossi accanto a "Giornata N" (max 6 + "+N", "0 vite" se eliminato). Verificato: G1=3❤ G2=1❤ per chi ha perso 2 vite in G1 (screenshot OK).
