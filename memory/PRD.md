...
- Importato il calendario completo dall'Excel utente (`Calendario_SerieA_2026-27`): **380 partite, 38 giornate, tutte con 10 partite**. Parser custom su fogli ANDATA/RITORNO (blocchi di 3 giornate affiancate). Import diretto in `sal_calendar` (wipe stagione + reload). Verificato: nuovo torneo su G1 → 10 partite.
- 3 giornate con incoerenza NEL FILE sorgente (una squadra 2 volte, una assente) — in attesa conferma utente: G4 (Roma 2x, manca Cagliari), G20 (Milan 2x, manca Roma), G22 (Genoa 2x, manca Roma). Importate comunque a 10 partite ma da correggere.
